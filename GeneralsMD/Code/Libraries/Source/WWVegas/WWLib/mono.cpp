...
{
#ifdef _WINDOWS
	Handle = CreateFile("\\\\.\\MONO", GENERIC_READ|GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
	if (Current == nullptr) {
		Current = this;
	}
#endif
...
void MonoClass::Sub_Window(int x, int y, int w, int h)
{
	...
	if ( Enabled && (Handle != INVALID_HANDLE_VALUE) ) {
		...
		subwindow.W = w;
		subwindow.H = h;
		DeviceIoControl(Handle, (DWORD)IOCTL_MONO_SET_WINDOW, &subwindow, sizeof(subwindow), nullptr, 0, &retval, 0);
	}
#endif
}
...
void MonoClass::Set_Cursor(int x, int y)
{
	...
	if ( Enabled && (Handle != INVALID_HANDLE_VALUE) ) {
		...
		cursor.X = x;
		cursor.Y = y;
		DeviceIoControl(Handle, (DWORD)IOCTL_MONO_SET_CURSOR, &cursor, sizeof(cursor), nullptr, 0, &retval, 0);
	}
#endif
}
...
void MonoClass::Fill_Attrib(int x, int y, int w, int h, MonoAttribute attrib)
{
	...
	if ( Enabled && (Handle != INVALID_HANDLE_VALUE) ) {
		...
		DeviceIoControl(Handle, (DWORD)IOCTL_MONO_FILL_ATTRIB, &fillcontrol, sizeof(fillcontrol), nullptr, 0, &retval, 0);
	}
#endif
}
