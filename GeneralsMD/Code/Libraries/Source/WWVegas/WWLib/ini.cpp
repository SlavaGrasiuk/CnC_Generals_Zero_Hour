...
const int INIClass::MAX_LINE_LENGTH = 4'096;
 ...
 *                                                                                             *
 *    This routine will store a floating point number to the section and entry of the          *
 *    database.                                                                                *
 *                                                                                             *
 * INPUT:   section  -- The section to store the entry under.                                  *
 *                                                                                             *
...
int INIClass::Get_String(char const* section, char const* entry, char const* defvalue, char* buffer, int size) const {
    ...
    */
    if (defvalue == nullptr) {
        ...
        strncpy_s(buffer, size, defvalue, size);
        buffer[size - 1] = '\0';
        strtrim(buffer);
...
int INIClass::Get_Int_Bitfield(char const* section, char const* entry, int defvalue, char* list[]) {
    ...
    // int count	= 0; (gth) initialized but not referenced...
    int retval = 0;
    char* str = _strdup(entryptr->Value);

    int lp;
    for (char* token = strtok_s(str, "|+"); token; token = strtok_s(nullptr, "|+")) {
        for (lp = 0; list[lp]; lp++) {
...
int* INIClass::Get_Alloc_Int_Array(char const* section, char const* entry, int listend) {
    ...
    char* str = _strdup(entryptr->Value);
    char* token;
    for (token = strtok_s(str, " "); token; token = strtok_s(nullptr, " ")) {
        count++;
    }
    free(str);
...
void INIClass::DuplicateCRCError(const char* message, const char* section, const char* entry) {
    char buffer[512];
    _snprintf_s(buffer, sizeof(buffer), "%s - Duplicate Entry \"%s\" in section \"%s\" (%s)\n", message,
