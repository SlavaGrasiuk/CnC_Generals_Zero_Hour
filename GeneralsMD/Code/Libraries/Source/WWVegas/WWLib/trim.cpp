...
char* strtrim(char* buffer) {
    if (buffer) {
        ...
        // (slavagrasiuk) workaround just to shut compiler up without using _CRT_SECURE_NO_WARNINGS; Too much work for proper fix is required, maybe later:(
        ...
        for (int index = bufferSize - 1; index >= 0; --index) {
...
wchar_t* wcstrim(wchar_t* buffer) {
    if (buffer) {
        ...
        // (slavagrasiuk) workaround just to shut compiler up without using _CRT_SECURE_NO_WARNINGS; Too much work for proper fix is required, maybe later:(
        const size_t bufferSize = wcslen(buffer);
