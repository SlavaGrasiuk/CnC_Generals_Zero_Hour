...
*/
static char ExceptionText[65'536];
...
static void Add_Txt(char const* txt) {
    if (strlen(ExceptionText) + strlen(txt) < 65'535) {
        strcat_s(ExceptionText, txt);
    }
#if (0)
    /*
...
void Dump_Exception_Info(EXCEPTION_POINTERS* e_info) {
    ...
    */
    if (e_info->ExceptionRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION) {
        sprintf_s(scrap, "Access address:%08X ", access_address);
        Add_Txt(scrap);
    ...
    */
    ...
    if (!IsBadCodePtr((FARPROC)context->Eip)) {
        if (_SymGetSymFromAddr != nullptr && _SymGetSymFromAddr(GetCurrentProcess(), context->Eip, &displacement, symptr)) {
            sprintf_s(scrap, "Exception occurred at %08X - %s + %08X\r\n", context->Eip, symptr->Name, displacement);
        } else {
            ...
            sprintf_s(scrap, "Exception occurred at %08X\r\n", context->Eip);
        }
    } else {
        ...
        for (int s = 0; s < num_addresses; s++) {
            ...
            if (symbols_available) {
                ...
                if (_SymGetSymFromAddr != nullptr && _SymGetSymFromAddr(GetCurrentProcess(), temp_addr, &displacement, symptr)) {
                    char symbuf[256];
                    sprintf_s(symbuf, "%s + %08X\r\n", symptr->Name, displacement);
                    Add_Txt(symbuf);
                }
            } else {
                char symbuf[256];
                sprintf_s(symbuf, "%08x\r\n", temp_addr);
                Add_Txt(symbuf);
            }
        }
    ...
    */
    for (int thread = 0; thread < ThreadList.Count(); thread++) {
        sprintf_s(scrap, "  ID: %08X - %s", ThreadList[thread]->ThreadID, ThreadList[thread]->ThreadName);
        Add_Txt(scrap);
    ...
    ** Dump the registers.
    */
    sprintf_s(scrap, "Eip:%08X\tEsp:%08X\tEbp:%08X\r\n", context->Eip, context->Esp, context->Ebp);
    Add_Txt(scrap);
    sprintf_s(scrap, "Eax:%08X\tEbx:%08X\tEcx:%08X\r\n", context->Eax, context->Ebx, context->Ecx);
    Add_Txt(scrap);
    sprintf_s(scrap, "Edx:%08X\tEsi:%08X\tEdi:%08X\r\n", context->Edx, context->Esi, context->Edi);
    Add_Txt(scrap);
    ...
    */
    ...
    sprintf_s(scrap, "     Control word: %08x\r\n", context->FloatSave.ControlWord);
    Add_Txt(scrap);
    sprintf_s(scrap, "      Status word: %08x\r\n", context->FloatSave.StatusWord);
    Add_Txt(scrap);
    sprintf_s(scrap, "         Tag word: %08x\r\n", context->FloatSave.TagWord);
    Add_Txt(scrap);
    sprintf_s(scrap, "     Error Offset: %08x\r\n", context->FloatSave.ErrorOffset);
    Add_Txt(scrap);
    sprintf_s(scrap, "   Error Selector: %08x\r\n", context->FloatSave.ErrorSelector);
    Add_Txt(scrap);
    sprintf_s(scrap, "      Data Offset: %08x\r\n", context->FloatSave.DataOffset);
    Add_Txt(scrap);
    sprintf_s(scrap, "    Data Selector: %08x\r\n", context->FloatSave.DataSelector);
    Add_Txt(scrap);
    //sprintf(scrap, "      Cr0NpxState: %08x\r\n", context->FloatSave.Cr0NpxState);
    sprintf_s(scrap, "      Cr0NpxState: %08x\r\n", context->FloatSave.Spare0);
    Add_Txt(scrap);

    for (int fp = 0; fp < SIZE_OF_80387_REGISTERS / 10; fp++) {
        sprintf_s(scrap, "ST%d : ", fp);
        Add_Txt(scrap);
        for (int b = 0; b < 10; b++) {
            sprintf_s(scrap, "%02X", context->FloatSave.RegisterArea[(fp * 10) + b]);
            Add_Txt(scrap);
        }
        ...
        if (IsBadReadPtr(stackptr, 4)) {
            ...
            */
            if (IsBadCodePtr((FARPROC)*stackptr)) {
                sprintf_s(scrap, "%08" PRIXPTR ": %08X ", uintptr_t(stackptr), *stackptr);
                ...
                if (symbols_available) {
                    ...
                    if (_SymGetSymFromAddr != nullptr && _SymGetSymFromAddr(GetCurrentProcess(), *stackptr, &displacement, symptr)) {
                        char symbuf[256];
                        sprintf_s(symbuf, " - %s + %08X", symptr->Name, displacement);
                        strcat_s(scrap, symbuf);
                    }
                } else {
                    strcat_s(scrap, " *");
                }
...
void Register_Thread_ID(unsigned long thread_id, char* thread_name, bool main_thread) {
    ...
    if (thread_name) {
        ...
        */
        ...
        strcpy_s(thread->ThreadName, thread_name);
 ...
 *=============================================================================================*/
bool Lookup_Symbol(void* code_ptr, char* symbol, const size_t symbolBufferSize, int& displacement) {
    /*
    ** Locals.
    */
    ...
    */
    if (IsBadCodePtr((FARPROC)code_ptr)) {
        strcpy_s(symbol, symbolBufferSize, "Bad code pointer");
        return false;
    }
    ...
    */
    if (_SymGetSymFromAddr(GetCurrentProcess(), (unsigned long)code_ptr, (unsigned long*)&displacement, symbol_struct_ptr)) {
        ...
        */
        strcpy_s(symbol, symbolBufferSize, symbol_struct_ptr->Name);
        return true;
    }
...
void Set_Exit_On_Exception(const bool set) {
