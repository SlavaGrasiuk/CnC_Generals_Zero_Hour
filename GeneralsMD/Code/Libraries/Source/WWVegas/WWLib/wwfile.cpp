...
int FileClass::Printf(char* str, ...) {
    ...
    va_list args;
    va_start(args, str);
    int length = _vsnprintf_s(text, PRINTF_BUFFER_SIZE, _TRUNCATE, str, args);
    va_end(args);
...
int FileClass::Printf(char* buffer, int bufferSize, char* str, ...) {
    va_list args;
    va_start(args, str);
    int length = _vsnprintf_s(buffer, bufferSize, _TRUNCATE, str, args);
    va_end(args);
...
int FileClass::Printf_Indented(unsigned depth, char* str, ...) {
    ...
    int length;
    if (depth < PRINTF_BUFFER_SIZE)
        length = _vsnprintf_s(text + depth, PRINTF_BUFFER_SIZE - depth, _TRUNCATE, str, args);
    else
