...
int	RegistryClass::Get_Int(const char* name, int def_value) {
    ...
    if ((::RegQueryValueEx((HKEY)Key, name, nullptr, &type, (LPBYTE)&data, &data_len) == ERROR_SUCCESS) && (type == REG_DWORD)) {
    } else {
        data = def_value;
    }
    return data;
...
char* RegistryClass::Get_String(const char* name, char* value, int value_size, const char* default_string) {
    assert(IsValid);
    DWORD type = 0;
    if ((::RegQueryValueEx((HKEY)Key, name, nullptr, &type, (LPBYTE)value, (DWORD*)&value_size) == ERROR_SUCCESS) && (type == REG_SZ)) {
    } else {
        //*value = 0;
        ...
        if (default_string == nullptr) {
            *value = 0;
            ...
            strcpy_s(value, value_size, default_string);
        }
    }
    return value;
}
    ...
    while (result == ERROR_SUCCESS) {
        ...
        if (result == ERROR_SUCCESS) {
            switch (type) {
                ...
                */
                case REG_DWORD:
                    strcpy_s(save_name, "DWORD_");
                    strcat_s(save_name, value_name);
        ...
        while (result == ERROR_SUCCESS) {
            ...
            if (result == ERROR_SUCCESS) {
                ...
                */
                ...
                strcpy_s(new_key_path, path);
                strcat_s(new_key_path, "\\");
                strcat_s(new_key_path, name);
    ...
    if (!IsLocked) {
        ...
        for (INISection* section = section_list.First(); section != nullptr; section = section->Next_Valid()) {
            ...
            */
            ...
            strcpy_s(path, new_path);
            char* cut = strstr(section_name, old_path);
            if (cut) {
                strcat_s(path, cut + old_path_len);
            }
