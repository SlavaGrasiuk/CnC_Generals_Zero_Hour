...
bool HMorphAnimClass::Import(const char *hierarchy_name, TextFileClass &text_desc)
{
	...
	//
	::strncpy_s (HierarchyName, hierarchy_name, W3D_NAME_LEN);
...
void HMorphAnimClass::Set_Name(const char * name)
{
	...
	if (separator != nullptr) {
		...
		//
		separator[0] = 0;
		::strcpy_s (AnimName, separator + 1);
		::strcpy_s (HierarchyName, full_name);
	}
...
int HMorphAnimClass::Load_W3D(ChunkLoadClass & cload)
{
	...
	strncpy_s(AnimName,header.Name,sizeof(AnimName));
    strncpy_s(HierarchyName,header.HierarchyName,sizeof(HierarchyName));
	strcpy_s(Name,HierarchyName);
	strcat_s(Name,".");
	strcat_s(Name,AnimName);
...
int HMorphAnimClass::Save_W3D(ChunkSaveClass & csave)
{
	...
	strncpy_s(header.Name,AnimName,sizeof(header.Name));
	strncpy_s(header.HierarchyName,HierarchyName,sizeof(header.HierarchyName));
