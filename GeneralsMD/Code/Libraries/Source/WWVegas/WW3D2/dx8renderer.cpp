...
void DX8TextureCategoryClass::Add_Polygon_Renderer(DX8PolygonRendererClass* p_renderer, DX8PolygonRendererClass* add_after_this) {
    ...
    if (add_after_this != nullptr) {
        const bool res = PolygonRendererList.Add_After(p_renderer, add_after_this, false);
        WWASSERT(res);
    } else {
...
//'Generals' customization to allow more than 65535 vertices
    ...
    if (maxVertexCount > 65535) {	//clamp vertex count to maximum size that can be indexed by 16-bit index
    ...
    while (renderedVertexCount < VisibleVertexCount) {
        mesh = remainingMesh;
        {
            ...
            while (mesh != nullptr) {
                ...
                if (vertex_offset + mesh_vertex_count > maxVertexCount || remainingMesh) {	//flag mesh so we know it didn't fit in the vertex buffer
        ...
        */
        ...
        if (!DX8RendererDebugger::Is_Enabled() || !mesh->Is_Disabled_By_Debugger()) {
            ...
            if ((!!mesh->Peek_Model()->Get_Flag(MeshGeometryClass::SORT)) && WW3D::Is_Sorting_Enabled()) {
                ...
                //if necessary
                if (mesh->Get_Alpha_Override() != 1.0 || (mesh->Get_User_Data() && *(int*)mesh->Get_User_Data() == RenderObjClass::USER_DATA_MATERIAL_OVERRIDE)) {	//mesh has material override of some kind
                    ...
                    if (mesh->Get_Alpha_Override() != 1.0) {
                        if (mesh->Is_Additive()) {	//additvie blended mesh can't switch to alpha or we will get a black outline.
